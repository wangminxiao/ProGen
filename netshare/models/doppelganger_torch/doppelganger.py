...
class DoppelGANger(object):
    ...
    def train(self, epochs, data_feature, data_attribute, data_gen_flag):
        ...
        self.check_data()
        dataset_benign = TensorDataset(
            torch.Tensor(data_attribute[:self.split_len]), torch.Tensor(data_feature[:self.split_len])
        ) #wmx
        dataset_malicious = TensorDataset(
            torch.Tensor(data_attribute[self.split_len:]), torch.Tensor(data_feature[self.split_len:])
        ) #wmx

        self._train(dataset_benign, dataset_malicious)

    def generate(
        self,
        data_feature, data_attribute, data_gen_flag, # wmx
        num_samples,
        ...
        #dataset_benign = TensorDataset(
        #    torch.Tensor(data_attribute[:5945]), torch.Tensor(data_feature[:5945])
        #)
        dataset_malicious = TensorDataset(
            torch.Tensor(data_attribute[self.split_len:]), torch.Tensor(data_feature[self.split_len:]) #wmx
        )

        loader_malicious = DataLoader(
            dataset_malicious,
        ...
        # end orig
        generated_data_list = []
        #for n_batch in range(num_batches): wmx
        for n_batch, (real_attribute_m, real_feature_m) in enumerate(loader_malicious):
            # start
            ...
            if given_attribute is not None and given_attribute is not None:
                batch_given_attribute = given_attribute[
                    n_batch * len(real_attribute_m): (n_batch + 1) * len(real_attribute_m)
                ]
            ...
            generated_data_list.append(
                self._generate(
                    real_attribute_m=real_attribute_m,
                    real_feature_m=real_feature_m,
                    real_attribute_noise=real_attribute_noise,
                    #[n_batch * self.batch_size: (n_batch + 1) * self.batch_size], #wmx
        ...
        print('generated_data_list', len(generated_data_list))
    ...
    def _build(self):
        ...
        self.generator = DoppelGANgerGenerator(
            attr_latent_dim=self.attribute_latent_dim + 187, #wmx +
    ...
    def _train(self, dataset_benign, dataset_malicious):
        ...
        for epoch in tqdm(range(self.epochs)):
            ...
            for batch_idx, ((real_attribute_b, real_feature_b), (real_attribute_m, real_feature_m)) in enumerate(zip(loader_benign, loader_malicious)): #

                real_attribute_b = real_attribute_b.to(self.device)
                real_feature_b = real_feature_b.to(self.device)

                real_attribute_m = real_attribute_m.to(self.device)
                real_feature_m = real_feature_m.to(self.device)
                ...
                for _ in range(self.d_rounds):
                    fake_attribute_list = []
                    fake_feature_list = []
                    gen_input_real_feature_m = []
                    gen_input_real_attribute_m = []
                    for _ in range(self.num_packing):
                        ...
                        gen_input_real_feature_m.append(real_feature_m) #wmx
                        gen_input_real_attribute_m.append(real_attribute_m)#wmx

                    packed_fake_attribute = torch.cat(
                        fake_attribute_list, dim=1)
                    packed_fake_feature = torch.cat(fake_feature_list, dim=1)
                    packed_gen_input_real_feature_m = torch.cat(gen_input_real_feature_m, dim=1) #wmx
                    packed_gen_input_real_attribute_m = torch.cat(gen_input_real_attribute_m, dim=1) #wmx

                    num_attrs = real_attribute_b.size()[1]
                    packed_real_attribute = real_attribute_b.view(
                        -1, self.num_packing * num_attrs
                    )

                    num_time_steps = real_feature_b.size()[1]
                    num_feats = real_feature_b.size()[2]
                    packed_real_feature = real_feature_b.view(
                ...
                for _ in range(self.g_rounds):
                    ...
                    gen_constraint_loss = factor1 * (criterion(packed_fake_feature, packed_gen_input_real_feature_m)) + factor2 * (criterion(packed_fake_attribute, packed_gen_input_real_attribute_m)) # wmx
                    if self.use_attr_discriminator:
                        attr_dis_fake = self.attr_discriminator(
                            packed_fake_attribute)
                    ...
                    gen_loss = gen_loss + gen_constraint_loss
